...
def _test_range(dut_list: list[TABLE_INT]) -> bool:
    return_val = True
    for i in range(len(dut_list)):
        # makes sure all waves are within int16 range. Capped to 2^15 to avoid signed int over/underflow.
        if any([abs(i)>2**15 for i in dut_list[i]]):
            print(f"ERROR** Range Test -- Wave at position {i} in wave list has value(s) outside of range! Please regenerate that table and scale it to +- 2**15.")
            return_val = False 
    return return_val

def _test_int(dut_list: list[TABLE_INT]) -> bool:
    return_val = True
    for i in range(len(dut_list)):
        # Tests if all values in list are ints 
        if any([type(i) != int for i in dut_list[i]]):
            print(f"ERROR** Int Test -- Wave at position {i} in wave list has non-int value! Please regenerate that table and convert all values to int16.")
            return_val = False 
    return return_val

def _test_peak(dut_list: list[TABLE_INT]) -> bool:
    return_val = True
    for i in range(len(dut_list)):
        # Tests if wave has peak at 2**15 +- 1 (to account for rounding errors) 
        if max(dut_list[i]) not in [2**15-1, 2**15, 2**15+1]:
            print(f"ERROR** Peak Test -- Wave at position {i} in wave list has not been scaled properly! Please regenerate that table and scale it to +- 2**15.")
            return_val = False 
    return return_val
